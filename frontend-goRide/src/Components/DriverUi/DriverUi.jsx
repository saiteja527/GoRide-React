...
const DriverUi = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await axios.post('http://localhost:5000/api/auth/driver', formData);
    ...
    const acceptRide = async (id) => {
        ...
        const res = await axios.put(`http://localhost:5000/api/auth/acceptRide/${id}/${formData.name}/${formData.mobileNumber}`);
        console.log(res);
    ...
    return (
        <div className='driverui'>
            ...
            <div className="data-right-container">
                ...
                {bookings.map((item) => (
                    !item.ride && (
                        <div className="request-item" style={{ marginBottom: 30 }}>
                            <div key={item.id} className="request-items">
                                <div className="user">
                                    <p><strong>UserName:</strong> {item.username}</p>
                                    <p><strong>Phone No:</strong> {item.phone}</p>
                                    <p><strong>Pickup:</strong> {item.source}</p>
                                    <p><strong>DropOff:</strong> {item.destination}</p>
                                    <button type="button" onClick={() => acceptRide(item._id)}>Accept ride</button>
                                </div>
                            </div>
                        </div>
                    )
